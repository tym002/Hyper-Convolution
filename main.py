...
def TrainandValidate(gpu):  
    ...
    model = unet(img_shape=(128,128,4), # size of the input image
                depth=4, # nums of maxpooling layers
                dropout=0.5,
                activation='relu',
                start_ch = 32, # initial channels
                ...
                hyper=False) # Hyper-conv kernel size. Use False for regular network
    ...
    mtrain = model.fit_generator(train_generator, steps_per_epoch=len(x_train)//b_size,
                                 epochs=1000, verbose=1, shuffle=True,callbacks=[model_checkpoint],validation_data=(x_test, [y_test]))
...
def prediction(gpu):
    ...
    ##############################
    model = unet(img_shape=(128,128,4),
